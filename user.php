		...
		<div class="doc auth">
			...
			<div class="page auth">
				...
				';
						...
						$res = mysqli_query($link, "SELECT * FROM users_prog WHERE `nick`='".$_GET['login']."'");
						...
						if(mysqli_num_rows($res) == 0 || ($user['pass'] != $_GET['pass'])) {
							...
							if($_GET['page']=="users" && $user['AL'] <= 0) {
								...
								";
								$res = mysqli_query($link, "SELECT * FROM users_prog");
							...
							if($_GET['page']=="user") {
								...
								$sc = 0;
								$i = 0;
								$res = mysqli_query($link, "SELECT * FROM task_prog");
								while($row = mysqli_fetch_array($res)) {
									if($user['t'.$i] == '1') {
							...
							if($_GET['page']=="adduser" && $user['AL'] <= 0) {
								...
								if($_GET['page']=="adduserconf" && $user['AL'] <= 0) {
									if(mysqli_num_rows(mysqli_query($link, "SELECT * FROM users_prog WHERE `nick`='".$_GET['param1']."'"))==0) {
									$res = mysqli_query($link, "SELECT * FROM users_prog");
									$id = mysqli_num_rows($res);
									$res = mysqli_query($link, "INSERT INTO users_prog (`id`, `name`, `nick`, `pass`, `AL`) VALUES ('".$id."', '".$_GET['param2']."', '".$_GET['param1']."', '123', 1)");
									echo '<p>ГОТОВО.</p>';
									redirect("users");
									}
									else {
							...
							if($_GET['page']=="addtask" && $user['AL'] <= 0) {
								...
								if($_GET['page']=="addtaskconf" && $user['AL'] <= 0) {
									$res = mysqli_query($link, "SELECT * FROM task_prog");
									$id = mysqli_num_rows($res);
									$res = mysqli_query($link, "INSERT INTO task_prog (`id`, `name`, `text`, `image`, `themes`, `langs`, `DM`, `DL`, `MU`, `author`) VALUES ('".$id."', '".$_GET['param1']."', '".$_GET['param2']."', '".$_GET['param3']."', '".$_GET['param4']."', '".$_GET['param5']."', '".$_GET['param6']."', '".$_GET['param7']."', '".$_GET['param8']."', '".$_GET['param9']."')");
									$res = mysqli_query($link, "ALTER TABLE `users_prog` ADD `t".$id."` INT NOT NULL ;");
									echo '<p>ГОТОВО.</p>';
									redirect("tasks");
								}
							if($_GET['page']=="taskend" && $user['AL'] <= 0) {
								...
								if($_GET['page']=="taskendconf" && $user['AL'] <= 0) {
									$res = mysqli_query($link, "SELECT * FROM users_prog");
									$id = mysqli_num_rows($res);
									$res = mysqli_query($link, "UPDATE users_prog SET `".$_GET['param2']."`='1' WHERE `name`='".$_GET['param1']."'");
							...
							if($_GET['page']=="taskview") {
								startBlock();
									...
									$res1 = mysqli_query($link, "SELECT * FROM task_prog WHERE id=".$_GET['param1']);
							...
							if($_GET['page']=="newpass") {
								...
								if($_GET['page']=="newpassconf") {
									$res = mysqli_query($link, "UPDATE users_prog SET `pass`='".$_GET['param1']."' WHERE `id`='".$user[id]."'");
									//echo "UPDATE users_prog SET `".$_GET['param2']."`='1' WHERE `name`='".$_GET['param1']."'";
									echo '<p>ГОТОВО.</p>';
									redirect("exit");
								}
						...
						if($_GET['page']=="ranking") {
							startBlock();
								...
								<script> users = [';
								$res1 = mysqli_query($link, "SELECT * FROM users_prog");
					...
					if($_GET['login']!="") mysqli_close($link);
